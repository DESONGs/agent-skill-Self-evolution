...
class ReviewServiceTest {
    ...
    void setUp() {
        ...
        reviewService = new ReviewService(
                reviewTaskRepository, skillVersionRepository, securityAuditRepository, skillRepository,
    ...
    private SecurityAudit createSafeAudit() {
        SecurityAudit audit = new SecurityAudit(SKILL_VERSION_ID, ScannerType.SKILL_SCANNER);
        audit.setVerdict(SecurityVerdict.SAFE);
        audit.setIsSafe(true);
        audit.setScannedAt(Instant.now(CLOCK));
        return audit;
    }

    private SecurityAudit createUnsafeAudit(SecurityVerdict verdict) {
        SecurityAudit audit = new SecurityAudit(SKILL_VERSION_ID, ScannerType.SKILL_SCANNER);
        audit.setVerdict(verdict);
        audit.setIsSafe(false);
        audit.setScannedAt(Instant.now(CLOCK).minusSeconds(60));
        return audit;
    }

    private void stubSafeReviewAudit() {
        when(securityAuditRepository.findAllActiveBySkillVersionId(SKILL_VERSION_ID))
                .thenReturn(List.of(createSafeAudit()));
    }

    private void stubUnsafeReviewAudit(SecurityVerdict verdict) {
        when(securityAuditRepository.findAllActiveBySkillVersionId(SKILL_VERSION_ID))
                .thenReturn(List.of(createUnsafeAudit(verdict)));
    }
    ...
    class ApproveReview {
        ...
        @Test
        void shouldRejectApproveWhenScanFailedWithoutSafeAudit() {
            ReviewTask task = createPendingReviewTask();
            Namespace ns = createTeamNamespace();
            SkillVersion sv = createSkillVersionWithStatus(SkillVersionStatus.SCAN_FAILED);
            stubUnsafeReviewAudit(SecurityVerdict.DANGEROUS);
        ...
        @Test
        void shouldRejectApproveWhenLatestAuditIsNotSafe() {
            ReviewTask task = createPendingReviewTask();
            Namespace ns = createTeamNamespace();
            SkillVersion sv = createPendingReviewSkillVersion();
            Skill skill = createSkill();
            stubUnsafeReviewAudit(SecurityVerdict.SUSPICIOUS);
        ...
        @Test
        void shouldRejectApproveWhenSecurityAuditMissing() {
            ReviewTask task = createPendingReviewTask();
            Namespace ns = createTeamNamespace();
            SkillVersion sv = createPendingReviewSkillVersion();
