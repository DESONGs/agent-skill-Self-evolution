...
class PromotionServiceTest {
    ...
    @Mock private SkillBundleArtifactRepository skillBundleArtifactRepository;
    @Mock private SkillEnvironmentProfileRepository skillEnvironmentProfileRepository;
    @Mock private SkillActionRepository skillActionRepository;
    @Mock private SkillEvalSuiteRepository skillEvalSuiteRepository;
    ...
    void setUp() {
        promotionService = new PromotionService(
                ...
                skillFileRepository, skillBundleArtifactRepository, skillEnvironmentProfileRepository,
                skillActionRepository, skillEvalSuiteRepository, namespaceRepository,
                permissionChecker, eventPublisher, governanceNotificationService, CLOCK);
        lenient().when(skillBundleArtifactRepository.findBySkillVersionId(anyLong())).thenReturn(Optional.empty());
        lenient().when(skillEnvironmentProfileRepository.findBySkillVersionId(anyLong())).thenReturn(List.of());
        lenient().when(skillActionRepository.findBySkillVersionId(anyLong())).thenReturn(List.of());
        lenient().when(skillEvalSuiteRepository.findBySkillVersionId(anyLong())).thenReturn(List.of());
    }
    ...
    class ApprovePromotion {
        ...
        void shouldApprovePromotionSuccessfully() {
            ...
            // Verify new version created, then saved again after bundle metadata copy.
            ArgumentCaptor<SkillVersion> versionCaptor = ArgumentCaptor.forClass(SkillVersion.class);
            verify(skillVersionRepository, times(2)).save(versionCaptor.capture());
            List<SkillVersion> savedVersions = versionCaptor.getAllValues();
            SkillVersion newVersion = savedVersions.get(0);
            ...
            assertEquals(NEW_VERSION_ID, savedVersions.get(1).getId());
