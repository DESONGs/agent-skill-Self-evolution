...
class SkillPublishServiceTest {
    ...
    @Mock
    private SkillBundleArtifactRepository skillBundleArtifactRepository;
    @Mock
    ...
    private SkillAssetNormalizationService skillAssetNormalizationService;

    @BeforeEach
    void setUp() {
        objectMapper = new ObjectMapper();
        skillAssetNormalizationService = new SkillAssetNormalizationService(objectMapper);
        ...
        lenient().when(skillBundleArtifactRepository.save(any(SkillBundleArtifact.class)))
                .thenAnswer(invocation -> invocation.getArgument(0));
        lenient().when(skillEnvironmentProfileRepository.save(any(SkillEnvironmentProfile.class))).thenAnswer(invocation -> {
            SkillEnvironmentProfile profile = invocation.getArgument(0);
            if (profile.getId() == null) {
                setId(profile, 100L);
            }
            return profile;
        });
        lenient().when(skillActionRepository.save(any(SkillAction.class))).thenAnswer(invocation -> invocation.getArgument(0));
        lenient().when(skillEvalSuiteRepository.save(any(SkillEvalSuite.class))).thenAnswer(invocation -> invocation.getArgument(0));
    }
    ...
    void testPublishFromEntries_Success() throws Exception {
        ...
        verify(skillBundleArtifactRepository).save(any(SkillBundleArtifact.class));
        verify(skillEnvironmentProfileRepository).save(any(SkillEnvironmentProfile.class));
