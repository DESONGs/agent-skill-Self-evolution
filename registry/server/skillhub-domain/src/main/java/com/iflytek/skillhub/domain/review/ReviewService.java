...
public class ReviewService {
    ...
    public ReviewTask approveReview(Long reviewTaskId, String reviewerId, String comment,
        ...
        assertLatestSecurityAuditSafe(skillVersion.getId());
    ...
    private void assertLatestSecurityAuditSafe(Long skillVersionId) {
        List<SecurityAudit> audits = securityAuditRepository.findAllActiveBySkillVersionId(skillVersionId);
        if (audits.isEmpty()) {
            throw new DomainBadRequestException("review.approve.scan_missing", skillVersionId);
        }

        SecurityAudit latestAudit = audits.stream()
                .max(Comparator.comparing(SecurityAudit::getScannedAt, Comparator.nullsLast(Comparator.naturalOrder()))
                        .thenComparing(SecurityAudit::getCreatedAt, Comparator.nullsLast(Comparator.naturalOrder())))
                .orElseThrow(() -> new DomainBadRequestException("review.approve.scan_missing", skillVersionId));

        if (latestAudit.getVerdict() != SecurityVerdict.SAFE) {
            throw new DomainBadRequestException(
                    "review.approve.scan_not_safe",
                    skillVersionId,
                    latestAudit.getVerdict() != null ? latestAudit.getVerdict().name() : null
            );
        }
    }
