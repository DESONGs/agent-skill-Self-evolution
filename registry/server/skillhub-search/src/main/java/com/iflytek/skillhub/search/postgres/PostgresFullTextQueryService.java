...
public class PostgresFullTextQueryService implements SearchQueryService {
    ...
    public SearchResult search(SearchQuery query) {
        ...
        if (query.labelSlugs() != null && !query.labelSlugs().isEmpty()) {
            ...
            sql.append("SELECT 1 FROM jsonb_array_elements_text(coalesce(d.label_slugs, '[]'::jsonb)) AS label_slug ");
            sql.append("WHERE LOWER(label_slug) IN :labelSlugs");
            sql.append(") ");
        }
        ...
        if ("downloads".equals(query.sortBy())) {
            sql.append("ORDER BY s.download_count DESC, d.published_at DESC, d.trust_score DESC, d.skill_id DESC ");
        } else if ("rating".equals(query.sortBy())) {
            sql.append("ORDER BY s.rating_avg DESC, d.published_at DESC, d.trust_score DESC, d.skill_id DESC ");
        } else if ("newest".equals(query.sortBy())) {
            sql.append("ORDER BY d.published_at DESC, d.trust_score DESC, d.skill_id DESC ");
            ...
            if (useShortPrefixTitleSearch) {
                sql.append("ts_rank_cd(").append(TITLE_VECTOR_SQL)
                        .append(", to_tsquery('simple', :tsQuery)) DESC, d.trust_score DESC, d.published_at DESC, d.skill_id DESC ");
            } else if (hasTsQuery) {
                sql.append("ts_rank_cd(d.search_vector, to_tsquery('simple', :tsQuery)) DESC, d.trust_score DESC, d.published_at DESC, d.skill_id DESC ");
            } else {
                sql.append("d.published_at DESC, d.trust_score DESC, d.skill_id DESC ");
            }
        } else {
            sql.append("ORDER BY d.published_at DESC, d.trust_score DESC, d.skill_id DESC ");
        }
