...
class SearchIndexEventListenerTest {
    ...
    @Test
    void activeStatusShouldTriggerRebuild() {
        SearchRebuildService searchRebuildService = mock(SearchRebuildService.class);
        SearchIndexService searchIndexService = mock(SearchIndexService.class);
        SearchIndexEventListener listener = new SearchIndexEventListener(searchRebuildService, searchIndexService);

        listener.onSkillStatusChanged(new SkillStatusChangedEvent(42L, SkillStatus.ARCHIVED, SkillStatus.ACTIVE));

        verify(searchRebuildService).rebuildBySkill(42L);
    }

    @Test
    void explicitRebuildEventShouldTriggerRebuild() {
        SearchRebuildService searchRebuildService = mock(SearchRebuildService.class);
        SearchIndexService searchIndexService = mock(SearchIndexService.class);
        SearchIndexEventListener listener = new SearchIndexEventListener(searchRebuildService, searchIndexService);

        listener.onSearchIndexSyncEvent(new SearchIndexSyncEvent(42L, SearchIndexAction.REBUILD));

        verify(searchRebuildService).rebuildBySkill(42L);
    }

    @Test
    void explicitRemoveEventShouldTriggerRemoval() {
        SearchRebuildService searchRebuildService = mock(SearchRebuildService.class);
        SearchIndexService searchIndexService = mock(SearchIndexService.class);
        SearchIndexEventListener listener = new SearchIndexEventListener(searchRebuildService, searchIndexService);

        listener.onSearchIndexSyncEvent(new SearchIndexSyncEvent(42L, SearchIndexAction.REMOVE));
