...
class PostgresFullTextQueryServiceTest {
    ...
    void shortPrefixRelevanceShouldRankUsingTitleVectorWithoutDuplicateOrderBy() {
        ...
        assertThat(sqlCaptor.getAllValues().getFirst())
                .contains("ts_rank_cd(to_tsvector('simple', coalesce(title, '')), to_tsquery('simple', :tsQuery))")
                .contains("d.trust_score DESC, d.published_at DESC, d.skill_id DESC");
    ...
    void labelFiltersShouldBeAppliedToSearchAndCountQueries() {
        ...
        verify(entityManager, org.mockito.Mockito.times(2)).createNativeQuery(sqlCaptor.capture());
        assertThat(sqlCaptor.getAllValues().getFirst())
                .contains("jsonb_array_elements_text(coalesce(d.label_slugs, '[]'::jsonb))")
                .contains("WHERE LOWER(label_slug) IN :labelSlugs");
        assertThat(sqlCaptor.getAllValues().getFirst()).doesNotContain("JOIN label_definition");
    ...
    void downloadsSortShouldNotBindRelevanceOnlyParameters() {
        ...
        assertThat(sqlCaptor.getAllValues().getFirst())
                ...
                .contains("ORDER BY s.download_count DESC, d.published_at DESC, d.trust_score DESC, d.skill_id DESC");
    }
    ...
    void emptyKeywordRelevanceShouldUseStableNewestOrdering() {
        ...
        verify(entityManager, org.mockito.Mockito.times(2)).createNativeQuery(sqlCaptor.capture());
        assertThat(sqlCaptor.getAllValues().getFirst())
                .contains("ORDER BY d.published_at DESC, d.trust_score DESC, d.skill_id DESC");
    }
