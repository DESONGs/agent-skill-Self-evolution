...
3. `action_id` MUST reference a declared action on that skill. If unsure, use the skill's default action.
4. A skill CAN be used multiple times with different node IDs
5. `depends_on` contains node IDs, not skill names
6. No circular dependencies
7. Every plan must produce outputs that satisfy the original task
