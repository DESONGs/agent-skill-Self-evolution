...
class EngineService:
    def __init__(
        self,
        registry_service: RegistryService | str | Path,
        *,
        runtime_runner: RuntimeRunner | None = None,
        search_service: EngineSearchService | None = None,
    ):
        self.registry_service = registry_service if isinstance(registry_service, RegistryService) else RegistryService(registry_service)
    ...
    def execute_skill(
        ...
        **kwargs: Any,
        ...
        if projection.skill_type and normalized_request.environment_profile:
            normalized_profile = normalized_request.environment_profile.strip().lower()
            if normalized_profile and normalized_profile not in {projection.skill_type.lower(), "default"}:
                # Keep the contract permissive for now, but surface clear mismatches.
                pass
...
def open_engine(root: str | Path) -> EngineService:
    return EngineService(root)


def find_skill(root: str | Path, request: FindSkillRequest | dict[str, Any] | str, **kwargs: Any) -> dict[str, Any]:
    return open_engine(root).find_skill(request, **kwargs).to_dict()


def execute_skill(root: str | Path, request: ExecuteSkillRequest | dict[str, Any] | str, **kwargs: Any) -> dict[str, Any]:
    return open_engine(root).execute_skill(request, **kwargs).to_dict()
