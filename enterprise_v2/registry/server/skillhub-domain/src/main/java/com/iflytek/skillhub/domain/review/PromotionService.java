...
public class PromotionService {
    ...
    public PromotionRequest approvePromotion(Long promotionId, String reviewerId,
        ...
        copyBundleArtifact(sourceVersion.getId(), newVersionId);
        newVersion.setBundleReady(skillBundleArtifactRepository.findBySkillVersionId(newVersionId).isPresent());
        skillVersionRepository.save(newVersion);
        Map<Long, Long> profileIdMapping = copyEnvironmentProfiles(sourceVersion.getId(), newVersionId);
        copyActions(sourceVersion.getId(), newVersionId, profileIdMapping);
        copyEvalSuites(sourceVersion.getId(), newVersionId);
    ...
    private void copyBundleArtifact(Long sourceVersionId, Long targetVersionId) {
        skillBundleArtifactRepository.findBySkillVersionId(sourceVersionId).ifPresent(sourceArtifact -> {
            SkillBundleArtifact copiedArtifact = new SkillBundleArtifact(
                    targetVersionId,
                    sourceArtifact.getStorageKey(),
                    sourceArtifact.getContentType(),
                    sourceArtifact.getSha256(),
                    sourceArtifact.getSizeBytes(),
                    sourceArtifact.getBuildStatus(),
                    sourceArtifact.getBuiltBy()
            );
            copiedArtifact.setManifestDigest(sourceArtifact.getManifestDigest());
            copiedArtifact.setBuiltAt(sourceArtifact.getBuiltAt());
            skillBundleArtifactRepository.save(copiedArtifact);
        });
    }

    private Map<Long, Long> copyEnvironmentProfiles(Long sourceVersionId, Long targetVersionId) {
        Map<Long, Long> profileIdMapping = new LinkedHashMap<>();
        for (SkillEnvironmentProfile sourceProfile : skillEnvironmentProfileRepository.findBySkillVersionId(sourceVersionId)) {
            SkillEnvironmentProfile copiedProfile = new SkillEnvironmentProfile(
                    targetVersionId,
                    sourceProfile.getProfileKey(),
                    sourceProfile.getCreatedBy()
            );
            copiedProfile.setDisplayName(sourceProfile.getDisplayName());
            copiedProfile.setRuntimeFamily(sourceProfile.getRuntimeFamily());
            copiedProfile.setRuntimeVersionRange(sourceProfile.getRuntimeVersionRange());
            copiedProfile.setToolRequirementsJson(sourceProfile.getToolRequirementsJson());
            copiedProfile.setCapabilityTagsJson(sourceProfile.getCapabilityTagsJson());
            copiedProfile.setOsConstraintsJson(sourceProfile.getOsConstraintsJson());
            copiedProfile.setNetworkPolicy(sourceProfile.getNetworkPolicy());
            copiedProfile.setFilesystemPolicy(sourceProfile.getFilesystemPolicy());
            copiedProfile.setSandboxMode(sourceProfile.getSandboxMode());
            copiedProfile.setResourceLimitsJson(sourceProfile.getResourceLimitsJson());
            copiedProfile.setEnvSchemaJson(sourceProfile.getEnvSchemaJson());
            copiedProfile.setDefaultProfile(sourceProfile.isDefaultProfile());
            SkillEnvironmentProfile savedProfile = skillEnvironmentProfileRepository.save(copiedProfile);
            profileIdMapping.put(sourceProfile.getId(), savedProfile.getId());
        }
        return profileIdMapping;
    }

    private void copyActions(Long sourceVersionId, Long targetVersionId, Map<Long, Long> profileIdMapping) {
        for (SkillAction sourceAction : skillActionRepository.findBySkillVersionId(sourceVersionId)) {
            SkillAction copiedAction = new SkillAction(
                    targetVersionId,
                    sourceAction.getActionId(),
                    sourceAction.getActionKind(),
                    sourceAction.getEntryPath(),
                    sourceAction.getCreatedBy()
            );
            copiedAction.setDisplayName(sourceAction.getDisplayName());
            copiedAction.setRuntimeFamily(sourceAction.getRuntimeFamily());
            copiedAction.setEnvironmentProfileId(profileIdMapping.get(sourceAction.getEnvironmentProfileId()));
            copiedAction.setTimeoutSec(sourceAction.getTimeoutSec());
            copiedAction.setSandboxMode(sourceAction.getSandboxMode());
            copiedAction.setAllowNetwork(sourceAction.isAllowNetwork());
            copiedAction.setInputSchemaJson(sourceAction.getInputSchemaJson());
            copiedAction.setOutputSchemaJson(sourceAction.getOutputSchemaJson());
            copiedAction.setSideEffectsJson(sourceAction.getSideEffectsJson());
            copiedAction.setIdempotencyMode(sourceAction.getIdempotencyMode());
            copiedAction.setDefaultAction(sourceAction.isDefaultAction());
            skillActionRepository.save(copiedAction);
        }
    }

    private void copyEvalSuites(Long sourceVersionId, Long targetVersionId) {
        for (SkillEvalSuite sourceSuite : skillEvalSuiteRepository.findBySkillVersionId(sourceVersionId)) {
            SkillEvalSuite copiedSuite = new SkillEvalSuite(
                    targetVersionId,
                    sourceSuite.getSuiteKey(),
                    sourceSuite.getSuiteType(),
                    sourceSuite.getGateLevel(),
                    sourceSuite.getCreatedBy()
            );
            copiedSuite.setDisplayName(sourceSuite.getDisplayName());
            copiedSuite.setEntryPath(sourceSuite.getEntryPath());
            copiedSuite.setConfigJson(sourceSuite.getConfigJson());
            copiedSuite.setSuccessCriteriaJson(sourceSuite.getSuccessCriteriaJson());
            copiedSuite.setLatestReportKey(sourceSuite.getLatestReportKey());
            skillEvalSuiteRepository.save(copiedSuite);
        }
    }
}
