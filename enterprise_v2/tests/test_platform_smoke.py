...
def test_registry_publish_install_feedback_and_promotion(tmp_path: Path) -> None:
    ...
    run_payload = platform.run_skill_lab_project(project_root)
    lineage = {
        "case_id": "case-registry-smoke",
        "proposal_id": "proposal-registry-smoke",
        "decision_mode": "patch",
    }
    submission = platform.build_promotion_submission(project_root, run_payload["run_id"], lineage=lineage)
    assert submission.lineage["case_id"] == "case-registry-smoke"
    assert submission.lineage["proposal_id"] == "proposal-registry-smoke"
    ...
    assert promotion_payload["lineage"]["decision_mode"] == "patch"

    with sqlite3.connect(registry_root / "registry.db") as conn:
        conn.row_factory = sqlite3.Row
        row = conn.execute(
            "select candidate_id, candidate_slug, run_id, case_id, proposal_id, decision_mode, lineage_json from promotion_requests order by id desc limit 1"
        ).fetchone()
    assert row["case_id"] == "case-registry-smoke"
    assert row["proposal_id"] == "proposal-registry-smoke"
    assert row["decision_mode"] == "patch"
    lineage_payload = json.loads(row["lineage_json"])
    assert lineage_payload["case_id"] == "case-registry-smoke"
    assert lineage_payload["proposal_id"] == "proposal-registry-smoke"
    assert lineage_payload["decision_mode"] == "patch"
...
def test_find_and_execute_skill_smoke(tmp_path: Path) -> None:
    platform = AgentSkillPlatform()
    package_root = ROOT / "tests" / "fixtures" / "github-pr-review"
    registry_root = tmp_path / "registry"

    platform.publish_package(package_root, registry_root=registry_root)

    search_payload = platform.find_skill(
        "GitHub pull request review",
        registry_root=registry_root,
    )
    assert search_payload["total_count"] >= 1
    assert search_payload["skills"][0]["skill_id"] == "github-pr-review"
    assert search_payload["skills"][0]["type"] == "script"
    assert "package_root" not in search_payload["skills"][0]

    execute_payload = platform.execute_skill(
        {
            "skill_id": "github-pr-review",
            "parameters": {"task": "review-pr"},
        },
        registry_root=registry_root,
    )
    assert execute_payload["ok"] is True
    assert execute_payload["execution"]["result"]["payload"]["summary"] == "review-pr"
    assert "package_root" not in execute_payload["execution"]["resolved_action"]
    assert "install_root" not in execute_payload["execution"]["resolved_action"]
